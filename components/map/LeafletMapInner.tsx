...
// Bold, high-contrast marker style: a solid-color pill with white text and
// a dark outline, so readings stand out against the light basemap — rather
// than the previous low-contrast "colored text on a white pill" treatment.
function dotIcon(color: string, size = 14, opts?: { label?: string; live?: boolean; ring?: boolean }) {
  const labelHtml = opts?.label
    ? `<span style="
        ...
        background:${color}; border:1.5px solid #111827; border-radius:9999px;
        padding:1px 6px 1px 5px; font-size:10px; font-weight:800; color:#fff;
        white-space:nowrap; box-shadow:0 1px 3px rgba(17,24,39,0.3);
      ">${
        opts.live
          ? `<span style="width:5px;height:5px;border-radius:9999px;background:#fff;box-shadow:0 0 0 2px rgba(255,255,255,0.4);"></span>`
          : ""
      }${opts.label}</span>`
    : "";
  return L.divIcon({
...
function riderIcon(color: string) {
  return L.divIcon({
    ...
    html: `<span style="
      ...
      box-shadow:0 2px 6px rgba(17,24,39,0.4), 0 0 0 2px #111827;
      ...
      >
        ...
        {markers.map((m) => (
          <Marker
            ...
            icon={dotIcon(m.color ?? "#059669", m.radius ?? 14, { label: m.label, live: m.live })}
          >
